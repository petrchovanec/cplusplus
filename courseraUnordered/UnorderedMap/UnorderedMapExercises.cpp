...
StringIntMap makeWordCounts(const StringVec& words) {
  ...
  for (auto &w : words) {
    if (!wordcount_map.count(w)){wordcount_map[w]=1;}else{wordcount_map[w]=wordcount_map[w]+1;}    
   }
...
int memoizedLongestPalindromeLength(LengthMemo& memo, const std::string& str, int leftLimit, int rightLimit, timeUnit startTime, double maxDuration) {
  ...
  if (false) {
    ...
    if (memo.count(std::make_pair(leftLimit,rightLimit))) {std::cout << "Memo: " << memo[std::make_pair(leftLimit,rightLimit)] << std::endl;};
  }
